...
function LeadContextBar({
  ...
  const chips = [
    budget   ? { icon: IndianRupee, label: formatCurrency(budget),   color: 'text-emerald-400' } : null,
    location ? { icon: MapPin,      label: location.split(',')[0],    color: 'text-white/50'    } : null,
    status   ? { icon: TrendingUp,  label: status.replace('-', ' '),  color: 'text-blue-400'    } : null,
  ...
  return (
    <div className="flex flex-wrap items-center gap-2 mb-5">
      ...
      {chips.map(({ icon: Icon, label, color }, i) => (
        <div
          key={i}
          className="flex items-center gap-1 bg-white/5 border border-white/8 rounded-full px-2.5 py-1"
        >
...
function CopyBtn({ text, size = 'sm' }: { text: string; size?: 'sm' | 'md' }) {
  ...
  return (
    <button
      onClick={copy}
      className={cn(
        'flex items-center gap-1.5 font-semibold rounded-lg border transition-all duration-150',
...
// ── Loading state — NO framer-motion transforms, CSS-only ─────────────────────
// The previous crash was caused by using translateX/translateY as plain CSS
// style properties inside a motion.div while also using animate x/y.
// This version uses only CSS animations — zero crash risk.

function ComposingLoader() {
  return (
    <div className="flex flex-col items-center justify-center py-14">
      {/* Pulsing brain icon — pure CSS animation */}
      <div className="relative mb-6">
        <div
          className="w-16 h-16 rounded-2xl bg-gradient-to-br from-emerald-500/20 to-teal-600/10 border border-emerald-500/20 flex items-center justify-center"
          style={{ animation: 'pulse 1.5s ease-in-out infinite' }}
        >
          <Brain className="w-8 h-8 text-emerald-400" />
        </div>
        {/* Three dots rotating — CSS only */}
        <div
    ...
    <div className="rounded-2xl border border-green-500/15 bg-[rgba(16,185,129,0.04)] overflow-hidden">
      ...
      <div className="p-4">
        ...
        {needsMore && (
          <button
            onClick={() => setExp(p => !p)}
            className="flex items-center gap-1 text-[10px] text-white/30 hover:text-white/55 mt-2.5 transition-colors"
          >
            {exp
    ...
    <div className="rounded-2xl border border-blue-500/15 bg-[rgba(59,130,246,0.04)] overflow-hidden">
      ...
      <div className="p-4 space-y-3">
        {sections.map((section, i) => {
          ...
          const isHdr   = heading?.endsWith(':') && !heading.startsWith('"') && !heading.startsWith('•')
          return (
            <div key={i}>
              {isHdr ? (
                <div>
          ...
          {!loading ? (
            <div className="flex items-center gap-3">
              <motion.button
                ...
                whileTap={{ scale: 0.97 }}
                className="flex items-center gap-2.5 px-6 py-3 rounded-xl font-bold text-[14px] text-white bg-gradient-to-r from-emerald-500 to-teal-500 shadow-[0_4px_24px_rgba(16,185,129,0.35)] hover:shadow-[0_6px_32px_rgba(16,185,129,0.5)] transition-shadow"
              >
          ...
          >
            ...
            <div className="flex items-center gap-3 px-1">
              ...
              {result.context.inactiveDays > 0 && (
                <span className="text-[10px] text-white/25">
                  {result.context.inactiveDays}d inactive · messages crafted accordingly
                </span>
              )}
            </div>
            ...
            {whatsappMsg && (
              <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 }}>
                <WhatsAppCard message={whatsappMsg.message} phone={leadPhone} />
              </motion.div>
            )}
            {callMsg && (
              <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.12 }}>
                <CallScriptCard message={callMsg.message} phone={leadPhone} />
              </motion.div>
            )}
            {emailMsg && (
              <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.19 }}>
                <EmailCard message={emailMsg.message} />
              </motion.div>
            )}
