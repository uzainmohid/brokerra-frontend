...
import { AiFollowUpComposer } from '@/components/leads/ai-followup-composer'
          ...
          >
            ...
            <div className="mt-5 pt-5 border-t border-white/6 grid grid-cols-2 sm:grid-cols-4 gap-4">
              {[
                ...
                { label: 'Property',    value: lead.propertyType ?? '—',                         color: 'text-white/80'    },
                { label: 'Added',       value: formatDate(lead.createdAt),                        color: 'text-white/60'    },
                { label: 'Last Active', value: formatRelative(lead.updatedAt),                    color: 'text-white/60'    },
              ].map(stat => (
                <div key={stat.label}>
